...
<div id="pt" class="bm cl">
	...
	<div class="y">
	    <!--{if rssforumperm($_G['forum']) && $_G[setting][rssstatus] && !$_GET['archiveid'] && !$subforumonly}--><a href="forum.php?mod=rss&fid=$_G[fid]&auth=$rssauth" class="fa_rss" target="_blank" title="RSS"></a><!--{/if}-->
        <span class="pipe">| </span>{lang rank}: <strong class="xi1" title="{lang previous_rank}:$_G[forum][oldrank]">$_G[forum][rank]</strong><!--{if $_G[forum][rank] <= $_G[forum][oldrank]}--><b class="ico_increase">&nbsp;</b><!--{else}--><b class="ico_fall">&nbsp;</b><!--{/if}-->
		<!--{if $_G['forum']['ismoderator']}-->
...
<div class="boardnav">
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		<!--{if $leftside}-->
			<div id="sd_bdl" class="bdl n5_lbbl" onmouseover="showMenu({'ctrlid':this.id, 'pos':'dz'});" style="width:{$_G['setting']['leftsidewidth']}px;margin-left:-{$_G['leftsidewidth_mwidth']}px">
	        <div id="side-wrap">
		        <div class="list-board">
                    <dl class="list-board-hd">
                        <i class="board-img">
                            <a href="forum.php?mod=forumdisplay&fid=$_G[fid]" title="$_G['forum'][name]"><img alt="$_G['forum'][name]" src="<!--{if $_G['forum'][icon]}-->data/attachment/common/$_G['forum'][icon]<!--{else}-->template/zhikai_n5chengshiv6/images/n5_zwtp.png<!--{/if}-->"><span class="img-shadow"></span></a>
                        </i>
                        <b>
                            <h1><a href="forum.php?mod=forumdisplay&fid=$_G[fid]" title=""><span>$_G['forum'][name]</span></a></h1>
					        <p>
                                <a class="J_followBoard n5-btn small" href="home.php?mod=spacecp&ac=favorite&type=forum&id=$_G[fid]&handlekey=favoriteforum&formhash={FORMHASH}">+收藏 <i id="number_favorite" {if !$_G[forum][favtimes]} style="display:none;"{/if}>(<span id="number_favorite_num">$_G[forum][favtimes]</span>)</i></a>
		                    </p>
                        </b>
                    </dl>
			        <div class="board-info">
                        ...
                        <p class="board-moderator link1">
                            <span class="color6">版主：</span><!--{if $moderatedby}-->$moderatedby<!--{else}--><a href="$_G['style']['列表边栏版主申请链接']" target="_blank">暂无版主 点击申请</a><!--{/if}-->
					    </p>
                        <span class="arrow">
                            <i class="arrow-top">◆</i>
                            <i>◆</i>
			        ...
			        <ul class="board-count color9">
				        <li>共有主题<em>$_G[forum][threads]</em></li>
				        <li>24时新帖<em>$_G[forum][todayposts]</em></li>
			        </ul>
                </div>
		        <!--{if $subexists && $_G['page'] == 1}-->
		            <div class="list-forum clearall">
			            <div class="link0 list-side-hd">
				            <h3 class="z n5_lbbt"><span>子</span>版块</h3>
			            </div>
			            <ul class="link1 list-side-bd ">
					        <!--{loop $sublist $sub}-->
                                ...
                                <li>
					                <i class="cforum-icon"><!--{if $sub[icon]}-->$sub[icon]<!--{else}--><!--{/if}--></i>
						            <h4><a target="_blank" ttname="threadlist_subforum" href="$forumurl" title="$sub[name]">$sub[name]</a></h4>
                                    <em>($sub[todayposts])</em>
						            <p>主题：<span>$sub[threads]</span></p>
						            <p>贴数：<span><!--{echo dnumber($sub[posts])}--></span></p>
					            </li>
                            <!--{/loop}-->
			            </ul>
		            </div>	
				...
				<div class="list-side">
					...
					<ul class="n5_hdlb">
					<!--[diy=n5_rmtw]--><div id="n5_rmtw" class="area"></div><!--[/diy]-->					
                    </ul>
		        </div>	
