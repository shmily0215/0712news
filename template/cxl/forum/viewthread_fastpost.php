...
<div id="f_pst" class="pl{if empty($_GET[from])} n5_nrftk bmw{/if}">
			...
			<div class="tedt{if !($_G[forum_thread][special] == 5 && empty($firststand))} mtn{/if}">
				<div class="bar">
					<span class="y">
						...
						使用 <a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]{if $_GET[from]}&from=$_GET[from]{/if}" onclick="return switchAdvanceMode(this.href)" style="color:#FF6600;"><b>{lang post_advancemode}</b></a>（可批量传图、插入视频等）
					</span>
